import { h as N } from "./chunk-KPMZENE5.mjs";
var g;
((e) => (e.log = N("\u{1F4A9} fallback-logger-please-reassign:services")))((g ||= {}));
((i) => {
  i._isTesting = !1;
  let e;
...
((u) => {
  let s;
  ((y) => (
    (y[(y.serviceNotFound = 404)] = "serviceNotFound"),
    (y[(y.serviceNotCompatible = 426)] = "serviceNotCompatible"),
    (y[(y.serviceGone = 410)] = "serviceGone"),
    (y[(y.implementation = 500)] = "implementation"),
    (y[(y.timedOut = 504)] = "timedOut"),
    (y[(y.badRequest = 400)] = "badRequest"),
    (y[(y.badResponse = 422)] = "badResponse")
  ))((s = u.Code ||= {}));
  class e extends u {
    code = 404;
  ...
  function c(a) {
    ...
    return (
      W(a) && (f.code = a.code),
      L(a) && (f.stack = a.stack),
      A(a) && (f.skipSentry = a.skipSentry),
      _(a) && (f.status = a.status),
      f
    );
  }
  ...
  function M(a) {
    if (a instanceof u)
      return { code: a.code, message: a.message, stack: a.stack, skipSentry: a.skipSentry };
    let v, f, F, B, q;
    return (
      typeof a == "string" ? (v = a) : j(a) && (v = a.message),
      L(a) && (f = a.stack),
      A(a) && (F = a.skipSentry),
      W(a) && (B = a.code),
      _(a) && (q = a.status),
      { code: B, message: v, stack: f, skipSentry: F, status: q }
    );
  }
  u.toMessageBody = M;
})((d ||= {}));
        ...
        c = async () => (
          this.cancelled ||
            (await new Promise((h, M) => {
              ((this.resolveCancelPromise = h), (this.rejectCancelPromise = M));
            })),
          n
        );
      return {
...
((t) => {
  let s;
  function e() {
    return ((s = s ?? new t()), g._sharedServiceManagerIfTesting() ?? s);
  }
  t.shared = e;
})((x ||= {}));
var P = class {
    ...
    async waitForDiscoveryInfo(e, t = 0) {
      ...
      for (; i <= t; )
        try {
          await this.postRequest(w.serviceId, { method: w.method }, r);
          break;
        } catch (n) {
    ...
    postRequest = (e, t, r, i) => {
      ...
      let o = {
        ...
        l = S.newResolvablePromise();
      ((this.waitingRequestsMap[o.id] = { result: l, onStreamValue: i }),
      ...
      return (
        ...
        Promise.race(c)
          .then((h) => h?.body)
          .catch((h) => {
            throw (delete this.waitingRequestsMap[o.id], h);
          })
      );
    };
    waitingRequestsMap = {};
    ...
    unregisterImplementation = async (e) => {
      ...
      let r = !1,
        i = [];
      for (let [n, o] of Object.entries(this.implementedServices))
        if (!(!t && o.rawImplementation !== e)) {
    ...
    onRequest = async (e) => {
      ...
      try {
        ...
        if (!e.stream) r = await l(e.body);
          ...
          let { id: c, method: h } = E.fromMessage(e.stream),
            M = this.requestedStreamsMap[c];
          if (h === "next") {
            if (!M) {
              let u = I.isServiceStreamOptions(e.body) ? e.body : void 0;
              ((M = {
                ...
                (this.requestedStreamsMap[c] = M));
            }
            try {
              let u = await M.iterator.next();
...
var k = class {
  ...
  ve = new k();
var p = typeof window > "u" ? void 0 : window,
  R = "data:origin-not-initialized",
  C = class s {
    constructor(e, t) {
      this.target = e;
      let r = s.targetRepresentingParentFrame;
      if (e === (p ? p.parent : void 0) || e === r) {
        if (!O || b !== void 0)
          throw new Error(
      ...
      t !== R && this.initializeTrustedOrigin(t);
    }
    target;
    get log() {
    ...
    static get toParentFrame() {
      return ((O = !0), (b = b ?? new s(s.targetRepresentingParentFrame, R)), (O = !1), b);
    }
    static targetRepresentingParentFrame = (() => {
      let e = "__targetRepresentingParentFrame",
        t = p?.[e] ?? p?.webkit?.messageHandlers?.[e];
      return {
        disabled: !t,
        postMessage: (...r) => {
          if (p) {
    ...
    initializeTrustedOrigin(e) {
      if (this.trustedOrigin !== R && !(this === b && e === this.trustedOrigin))
    ...
    addMessageListener(e) {
      (this.listeners.size === 0 && p?.addEventListener("message", this.onMessageEvent, !1),
        this.listeners.add(e));
    }
    removeMessageListener(e) {
      (this.listeners.delete(e),
        this.listeners.size === 0 && p?.removeEventListener("message", this.onMessageEvent, !1));
    }
    listeners = new Set();
    onMessageEvent = (e) => {
      ...
      if (e.source !== this.target)
        if (this === b && e.source === p && e.data?.__sourceRepresentsParentFrame) t = !0;
        else return;
  ...
  b,
  $ = C.toParentFrame,
  G = p && p.opener && p !== p.opener && p.parent === p ? new C(p.opener, R) : $;
var z;
((i) => {
  function s(n, o, l, c) {
    l({ method: n, argument: o ? c : void 0, oneway: !0 });
  }
  i.onewayMethodTemplate = s;
  async function e(n, o, l, c) {
    await l({ method: n, argument: o ? c : void 0 });
  }
  i.voidMethodTemplate = e;
  async function t(n, o, l, c) {
    return await l({ method: n, argument: o ? c : void 0 });
  }
  i.valueMethodTemplate = t;
  function r(n, o, l) {
    return new I.StreamReader(n, l, o);
  }
...
var H = class {
    ...
    onNewStream;
    newStream = (e) => {
      let t = S.generateUniqueId();
      return new D(
        (r, i) => {
          this.iterators.push({ id: t, update: r, done: i });
          let n = this.onNewStream?.(e);
          if (e?.replay === "latest") {
            let o = n?.latest ?? this.latestValue;
            if (o) r(o);
            else
              throw new d.Implementation(
        ...
        () => {
          let r = this.iterators.findIndex((i) => i.id === t);
          if (r >= 0) this.iterators.splice(r, 1);
          else
            throw new d.BadRequest(
              `ServiceEventEmitter couldn't find cancelled iterator with id: ${t}`
            );
        }
      );
    };
    ...
    emit = (e) => {
      ...
      for (let t of this.iterators) t.update(e);
    };
  ...
  D = class {
    constructor(e, t) {
      ...
      ((this.promises = [S.newResolvablePromise()]), e(this.update, this.update));
    }
    onIteratorEnd;
    log = g.log.extend("ServiceStreamIterator");
    hasAsyncIterator = !1;
    ...
    update = (e) => {
      let {
        ...
        promises: i,
        returnedNextPromise: n,
      } = this;
      if (!t) {
        if (!e || e instanceof d)
          throw new d.BadRequest("ServiceStream received return or throw before being read");
        r.push(e);
        return;
      }
      let o = i[this.promises.length - 1];
      if (e && o === void 0) {
        if (!n) {
      ...
      if (e === void 0) (o?.resolve(this.doneResult), n?.resolve(this.doneResult));
      else if (e instanceof d) n?.reject(e);
      else {
        if (this.onUpdate?.(e).ignore) return;
        (i.push(S.newResolvablePromise()), o?.resolve({ done: !1, value: e }));
      }
    };
    next = async () => {
    ...
    read = async (e) => {
      let t = this[Symbol.asyncIterator](),
        r = await t.next();
      for (; !r.done; ) (e(r.value), (r = await t.next()));
    };
    cancel = async () => {
      await this.return();
    };
  };
export { g as a, T as b, d as c, ee as d, x as e, ve as f, C as g, $ as h, G as i, z as j, H as k };
//# sourceMappingURL=https://app.framerstatic.com/chunk-U3GQEZQR.mjs.map
